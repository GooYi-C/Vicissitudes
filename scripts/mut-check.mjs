...
function runMutation(name, mutate) {
  ...
  try {
    ...
    for (const f of ['package.json', '.gitignore']) cpSync(join(root, f), join(tmp, f))
    ...
    console.log(`${caught ? '[ok]' : '[X]'} ${name} → static-checks ${caught ? '已捕获（非零退出）' : '未捕获（恒真空转！）'}`)
  } finally {
...
console.log(`\n[mut] PASS —— ${results.length}/${results.length} 故障全部被捕获，校验非空转`)
